...
def Splitmap(src_map, i):
    ...
    for case_path, case_name in src_map.items():
        path_arry = case_path.split(":")[0].split(".")
        if len(path_arry) > i + 1:
            if path_arry[i] not in dst_map:
                dst_map[path_arry[i]] = {}
            dst_map[path_arry[i]][case_path] = case_name
        else:
